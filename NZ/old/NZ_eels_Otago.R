...
# Save
Date = Sys.Date()
DateFile = paste0(main_dir,'/',Date,'_EEL_Otago/')
dir.create(DateFile)
setwd(DateFile)
...
network <- readRDS(file.path(data_dir, 'lower_otago_network_renumbered.rds'))
observations <- readRDS(file.path(data_dir, 'otago_observations_renumbered.rds'))
...
Opt = TMBhelper::Optimize( obj=Obj, lower=TmbList[["Lower"]], upper=TmbList[["Upper"]], getsd=TRUE, savedir=DateFile, loopnum=3, bias.correct=TRUE, bias.correct.control=list(sd=TRUE, split=NULL, nsplit=1, vars_to_correct="Index_cyl") )
...
## index of abundance
Index = plot_biomass_index( DirName=DateFile, TmbData=Data, Sdreport=Opt[["SD"]], Year_Set=Year_Set, Years2Include=Years2Include, use_biascorr=TRUE )
