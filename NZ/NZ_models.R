...
obs_enc <- readRDS(file.path(data_dir, "Waitaki_observations_encounters.rds"))
...
new_noise = ifelse( Data_Geostat[,'Catch_KG']>0.9, 1+0.01*rnorm(nrow(Data_Geostat)), 0 )
plot(Data_Geostat[,"Catch_KG"])
points(new_noise, col="blue")
Data_Geostat[,"Catch_KG"] <- new_noise
